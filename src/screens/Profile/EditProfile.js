      ...
      if (resp.status === 1) {
        if (selectedImage) {
          ...
          if (imguploadres.status !== 1) {
            ToastAndroid.show(imguploadres.Backend_Error, ToastAndroid.SHORT);
            return;
          }
        } else if (isDeleting) {
        ...
        ToastAndroid.show(resp.Backend_Error, ToastAndroid.SHORT);
      }
    } catch (err) {
  ...
  function openCamera() {
    ...
    launchCamera(options, res => {
      if (res.errorCode) {
        ...
        ToastAndroid.show(
          'Error in opening the Camera. Check your Permissions',
          ToastAndroid.SHORT,
        );
  ...
  function openGallery() {
    ...
    launchImageLibrary(options, res => {
      if (res.errorCode) {
        ...
        ToastAndroid.show(
          'Error in opening the gallery. Check your Permissions',
          ToastAndroid.SHORT,
        );
      } else if (!res.didCancel) {
        setSelectedImage(res.assets[0]);
      ...
      <SafeAreaView style={StyleConstants.safeArView}>
        ...
        <ScrollView>
          <View style={styles.Cview}>
            ...
            {nameErrorMessage && (
              <Text style={styles.errormsg} key={nameErrorMessage}>
                *{nameErrorMessage}
              </Text>
            )}
            ...
            {numErrorMessage && (
              <Text style={styles.errormsg} key={numErrorMessage}>
                *{numErrorMessage}
              </Text>
            )}
        ...
        <Overlay
          ...
          <View style={styles.logoutView}>
            <Text style={styles.logoutText}>
              *Entered details must be correct{' '}
            </Text>
