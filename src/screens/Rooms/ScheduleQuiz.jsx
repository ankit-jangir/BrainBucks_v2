...
export default function ScheduleQuiz({ navigation, route }) {
    ...
    const { loading, error, data, refetch } = useQuery(roomServ.GETEXAMCATEGORIES, {
        variables: {
            search_cat: searchCat, search_sub_cat: searchSubCat, cat_id: selectedCategory
        }
    })

    useEffect(() => {
        if (!data || !data.get_category_fromfill || !data.get_sub_category_fromfill) {
            return;
        }
        ...
        if (categoryData.error) {
            Toast.show({ type: "error", text1: categoryData.error })
            return;
        }
