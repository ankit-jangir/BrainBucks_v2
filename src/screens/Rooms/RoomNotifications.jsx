...
import { acceptJoinRequestInController, rejectJoinRequestInController } from '../../controllers/RoomsController';
...
export default function RoomNotifications({ navigation, route }) {
    ...
    async function action(user_id, type) {
        let res = type === "reject" ? await rejectJoinRequestInController(room_data._id, user_id, Toast) : await acceptJoinRequestInController(room_data._id, user_id, Toast);
        if (res) {
    ...
    useEffect(() => {
        if (data?.get_req_participants_of_quiz.error || error) {
            Toast.show({
                type: 'error',
                text1: error ? error : data?.get_req_participants_of_quiz.error
            })
            return;
        }
        ...
        if (data && data.get_req_participants_of_quiz) {
            ...
            if (currentPage === 1) {
                ...
                if (data.get_req_participants_of_quiz.response) {
                    setRequests([...requests, ...data.get_req_participants_of_quiz.response])
                }
            }
        }
    }, [data])
        ...
        <SafeAreaView style={{ backgroundColor: "white", flex: 1 }}>
            ...
            <View style={styles.notifiView}>
                <TouchableOpacity onPress={() => navigation.goBack()} style={styles.backBtn}>
                    <Image style={{ width: 20, height: 20 }} resizeMethod='contain' source={require('../../assets/img/arrow-left.png')} />
                </TouchableOpacity>
            ...
            </View>
            {
                loading
                    ...
                    requests.length === 0
                        ?
                        <NoDataFound scale={0.9} actionText={""} action={() => { }} message={"No Requests"} />
                        :
                        <FlatList
                            onEndReached={() => {
                            ...
                            renderItem={({ item, index }) => {
                                return (
                                    <View style={styles.requestView}>
                                        <View style={{ width: "20%" }}>
                                            <Image source={{ uri: BLOBURL + item.image }} style={styles.requestImage} />
                                        </View>
                                        <View style={{ width: '80%', gap: 20 }}>
                                            <Text style={styles.requesetName}>{item.name} Requested to join your room</Text>
            ...
            <Modal visible={modalVisible} onDismiss={() => { setModalVisible(false) }} contentContainerStyle={styles.modalContainer} >
                <LottieView source={require('../../assets/img/check.json')} autoPlay={true} style={styles.modalLottie} />
            </Modal>
        </SafeAreaView>
    )
}
