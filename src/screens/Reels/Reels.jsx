...
const Reels = ({navigation, route}) => {
  ...
  function toggleExpand(item) {
    setExpendedCaptions({
      ...expendedCaptions,
      [item._id]: !expendedCaptions[item._id],
    });
  }
  ...
  function getCurrentReelId() {
    ...
    let reelId = indexedReelId.substring(
      0,
      indexedReelId.length - currIndex.length,
    );
    return reelId;
  }
  ...
  async function deleteComment(comm) {
    ...
    let res = await BasicServices.apiTryCatch(async () => {
      return await reServ.deleteComment(currentId, comm);
    }, Toast);
    if (res) {
      getComments();
    }
  ...
  async function addComment() {
    ...
    let currentId = getCurrentReelId();
    let res = await BasicServices.apiTryCatch(async () => {
      return await reServ.addComment(currentId, comment);
    }, Toast);
    if (res) {
      getComments();
      setComment('');
          ...
          {tags.length === 0 ? (
            ...
            tags.map((item, index) => (
              <TouchableOpacity
                ...
                style={[
                  styles.tag,
                  !item.selected && {backgroundColor: 'white'},
                ]}>
                <Text
                  key={item._id + '' + item.selected}
                  style={{
                    color: item.selected ? 'white' : ColorsConstant.Black,
                  }}>
                  {item.selected && 'X  '}
                  {item.tag_name}
                </Text>
      ...
      <>
        {reels.length === 0 ? (
          ...
          <FlatList
            ...
            ref={scrollRef}
            onEndReached={() => {
              if (!loadingMore) getReels(currentPage + 1);
            }}
            data={reels}
            ...
            onViewableItemsChanged={({changed, viewableItems}) => {
              setCurrentReel(
                viewableItems[0]?.item?._id + viewableItems[0]?.index,
              );
              ...
              if (
                viewableItems[0] &&
                viewableItems[0].index === reels.length - 1
              ) {
              ...
              return (
                <Pressable
                  ...
                  <MemoizedVideo
                    ...
                    paused={paused}
                  />
                  <MemoizedCaptionSection
                    expendedCaptions={expendedCaptions}
                    item={item}
                    renderCaption={renderCaption}
                    toggleExpand={toggleExpand}
                  />
                  <MemoizedLikeCommentSection
                    onShare={onShare}
                    like={like}
                    item={item}
                        ...
                        {item.is_deletable && (
                          <TouchableOpacity
                            ...
                            <Image
                              ...
                              style={styles.deleteCommentIcon}></Image>
                          </TouchableOpacity>
                        )}
                      </View>
                    </View>
...
const MemoizedCaptionSection = memo(
  ({expendedCaptions, item, renderCaption, toggleExpand}) => {
    return (
      <View
        style={[
          styles.captionView,
        ...
        <ScrollView>
          <View style={styles.reelNameContainer}>
            ...
            />
            <Text key={item.name} style={styles.reelName}>
              {item.name}
            </Text>
          </View>
          <Text style={styles.caption} key={renderCaption(item)}>
            {renderCaption(item)}
          </Text>
          {renderCaption(item).length > 100 && (
            <TouchableOpacity
              onPress={() => toggleExpand(item)}
              style={{marginLeft: 20}}>
              <Text
                key={expendedCaptions[item._id] ? 'Read Less' : 'Read More'}
                style={{color: ColorsConstant.Theme, fontSize: 15}}>
                {expendedCaptions[item._id] ? 'Read Less' : 'Read More'}
              </Text>
            </TouchableOpacity>
          )}
        </ScrollView>
...
const MemoizedLikeCommentSection = memo(
  ({like, item, setModalVisible, onShare}) => {
    return (
      <LinearGradient
        ...
        <TouchableOpacity
          ...
          style={styles.likeItemContainer}>
          <Image
            source={require('../../assets/img/like.png')}
            tintColor={item.liked ? '#ED2839' : '#fff'}
            style={styles.likeIcon}
          />
          <Text key={item.likes} style={{color: ColorsConstant.White}}>
            {item.likes}
          </Text>
        </TouchableOpacity>
        ...
        <TouchableOpacity
          ...
          <Image
            source={require('../../assets/img/sharereel.png')}
            style={styles.likeIcon}
          />
